...
import { useTaskMyTasksQuery } from "@/hooks/useTask";
...
const myTasks = () => {
    ...
    const { data, isLoading } = useTaskMyTasksQuery<{ tasks: Task[] }>()
        ...
        myTasks?.length > 0
            ? myTasks
                .filter((task) => {
                    ...
                    if (filter === "todo") return task.status === TASK_STATUS.TO_DO;
                    if (filter === "inprogress") return task.status === TASK_STATUS.IN_PROGRESS;
                    if (filter === "done") return task.status === TASK_STATUS.DONE;
                    if (filter === "archieved") return task.isArchived === true;
                    if (filter === "high") return task.priority === TASK_PRIORITY.HIGH;
                    if (filter === "medium") return task.priority === TASK_PRIORITY.MEDIUM;
                    if (filter === "low") return task.priority === TASK_PRIORITY.LOW;
