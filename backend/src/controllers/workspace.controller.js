...
import { PROJECT_STATUS, TASK_STATUS } from "../utils/constant.js";
...
export const getWorkspaceStats = async (req, res) => {
    try {
        ...
        const totalTaskCompleted = projects.reduce((acc, project) => {
            return (
                acc + project.tasks.filter((task) => task.status === TASK_STATUS.DONE).length
            );
        }, 0);

        const totalTaskToDo = projects.reduce((acc, project) => {
            return (
                acc + project.tasks.filter((task) => task.status === TASK_STATUS.TO_DO).length
            );
        }, 0);
        ...
        const totalTaskInProgress = projects.reduce((acc, project) => {
            return (
                acc +
                project.tasks.filter((task) => task.status === TASK_STATUS.IN_PROGRESS).length
            );
        }, 0);
